...
function d0OnPath(): boolean {
  try {
    if (process.platform === "win32") {
      execSync("where d0", { stdio: "ignore", windowsHide: true });
    } else {
      execSync("which d0", { stdio: "ignore" });
    }
    return true;
  } catch {
    return false;
...
/**
 * How Cursor should spawn the d0 MCP server (stdio).
 * Prefer global `d0` when available; else `node` + this package's `dist/index.js`.
 */
export function resolveMcpLaunchConfig(): { command: string; args: string[] } {
  ...
  throw new Error(
    "Could not find d0 on PATH or dist/index.js. Install globally: npm install -g d0, or run npm run build from the d0 repo.",
  );
}
...
export async function cmdMcpInstall(opts: McpInstallOpts): Promise<void> {
  ...
  if (existsSync(targetPath)) {
    try {
      ...
      console.error(`d0 mcp install: invalid JSON in ${targetPath}`);
      process.exitCode = 1;
      return;
    }
  }
  ...
  if (prevD0 !== undefined && !opts.yes) {
    console.error("d0 mcp install: existing mcpServers.d0 differs. Re-run with --yes to replace.");
  ...
  console.log("Restart Cursor (or reload MCP) so the d0 server is picked up.");
}
